    ...
    public class PatientRepository : RepositoryBase<Patients>, IPatientRepository
    {
        ...
        private void MapPatientEncounter(PatientInfoDetails patientInfoDetails, PatientClinicalInformation patientClinicalInformation)
        {
            try
            {
                ...
                patientClinicalInformation.ptEncounters = patientInfoDetails.PatientEncounter.Select(d => new Encounters
                {
                    ...
                    Location = _context.MasterPatientLocation.Where(l => l.Id == d.ServiceLocationID).FirstOrDefault() != null ? _context.MasterPatientLocation.Where(l => l.Id == d.ServiceLocationID).FirstOrDefault().Location : _context.MasterPatientLocation.FirstOrDefault().Location,
        ...
        private void SavePatientVitals(int userID, PatientClinicalInformation patientClinicalInformation, Patients patient)
        {
            try
            {
                ...
                patientVitals = patientClinicalInformation.ptVitalSigns.Select(l => new PatientVitals
                {
                    ...
                    BMI = calculateBmi(Convert.ToString(l.HeightUnit).ToLower() == "cm" ? ConvertKgToLb(Convert.ToDouble(l.WEIGHT)) : Convert.ToDouble(l.WEIGHT),
                    Convert.ToString(l.WeightUnit).ToLower() == "kg" ? ConvertCentimetersToInches(Convert.ToDouble(l.Height)) : Convert.ToDouble(l.Height)),
                    //BPSystolic = Convert.ToInt32(l.BloodPressure.Split('/')[0]),
                    //BPDiastolic = Convert.ToInt32(l.BloodPressure.Split('/')[1]),
                    ...
                    IsDeleted = false,
                    HeartRate = 0,
                    HeightIn = Convert.ToString(l.HeightUnit).ToLower() == "cm" ? ConvertKgToLb(Convert.ToDouble(l.WEIGHT)) : Convert.ToDouble(l.WEIGHT),
                    Pulse = 0,
                    Respiration = 0,
                    Temperature = 0,
                    WeightLbs = Convert.ToString(l.WeightUnit).ToLower() == "kg" ? ConvertCentimetersToInches(Convert.ToDouble(l.Height)) : Convert.ToDouble(l.Height),
                    PatientID = patient.Id,
        ...
        private void SavePatientSocialHistory(int userID, PatientClinicalInformation patientClinicalInformation, Patients patient)
        {
            try
            {
                ...
                patientSocialHistory.AlcohalID = patientClinicalInformation.ptSocialHistory.Alcohol != null ? DefaultSocialStatus : 0;

                //patientSocialHistory.Occupation = "NA";
                patientSocialHistory.DrugID = patientClinicalInformation.ptSocialHistory.Drugs != null ? DefaultSocialStatus : 0;
                patientSocialHistory.TobaccoID = patientClinicalInformation.ptSocialHistory.Tobacoo != null ? DefaultSocialStatus : 0;
        ...
        private void SavePatientMedications(int userID, PatientClinicalInformation patientClinicalInformation, Patients patient)
        {
            try
            {
                ...
                patientMedication = patientClinicalInformation.ptMedication.Select(o => new Entity.PatientMedication
                {
                    ...
                    Strength = o.Strength,
                    FrequencyID = Convert.ToInt32(o.Frequency),
                }).ToList();
        ...
        private void SavePatientAddress(int userID, PatientClinicalInformation patientClinicalInformation, Patients patient)
        {
            try
            {
                ...
                patientAddress.PatientLocationID = patient.LocationID;
                ...
                patientAddress.AddressTypeID = _context.GlobalCode.Where(k => k.GlobalCodeCategoryID == _context.GlobalCodeCategory.Where(f => f.GlobalCodeCategoryName == "addresstype").FirstOrDefault().Id && k.GlobalCodeValue == "Primary Home").FirstOrDefault().Id;
        ...
        private Patients SavePatientDemographics(int organizationID, int userID, PatientClinicalInformation patientClinicalInformation)
        {
            try
            {
                ...
                patient.Race = _context.MasterRace.Where(k => k.RaceName.Contains(patientClinicalInformation.ptDemographicDetail.Race)).FirstOrDefault() != null ?
                    _context.MasterRace.Where(k => k.RaceName.Contains(patientClinicalInformation.ptDemographicDetail.Race)).FirstOrDefault().Id : _context.MasterRace.FirstOrDefault().Id;
                ...
                patient.Gender = _context.MasterGender.Where(l => l.Gender.Contains(patientClinicalInformation.ptDemographicDetail.gender)).FirstOrDefault() != null ?
                    _context.MasterGender.Where(l => l.Gender.Contains(patientClinicalInformation.ptDemographicDetail.gender)).FirstOrDefault().Id : _context.MasterGender.FirstOrDefault().Id;
        ...
        private Entity.User SaveUserInfo(int organizationID, int userID)
        {
            try
            {
                ...
                int patientRoleID = _context.UserRoles.Where(m => m.RoleName.ToLower() == "client" && m.OrganizationID == organizationID && m.IsActive == true && m.IsDeleted == false).FirstOrDefault().Id;
