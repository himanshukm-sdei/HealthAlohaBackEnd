...
namespace HC.Patient.Service.Services.Patient
{
    public class PatientService : BaseService, IPatientService
    {
        ...
        public PatientService(IPatientRepository patientRepository, IPatientAuthorizationRepository patientAuthorizationRepository, HCOrganizationContext context, IImageService imageService, IUserRepository userRepository, IOrganizationSMTPRepository organizationSMTPRepository, IEmailService emailSender, IHostingEnvironment env, ITokenService tokenService, IPatientDiagnosisService patientDiagnosisService, IPatientTagRepository patientTagRepository)
        {
        ...
        public JsonModel CreateUpdatePatient(PatientDemographicsModel patientDemographics, TokenModel token)
        {
            ...
            PHIEncryptedModel pHIEncryptedModel = _patientRepository.GetEncryptedPHIData<PHIEncryptedModel>(patientDemographics.FirstName, patientDemographics.MiddleName, patientDemographics.LastName, patientDemographics.DOB!=null?patientDemographics.DOB.ToString("yyyy-MM-dd HH:mm:ss.fffffff") :null, patientDemographics.Email, patientDemographics.SSN, patientDemographics.MRN, null, null, null, null, null, null, null).FirstOrDefault();
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (patientDemographics.Id == 0)
                    {
                        ...
                        string password = CommonMethods.Encrypt(patientDemographics.FirstName.ToUpper() + "@" + rnd.Next(1111, 9999));
        ...
        private void MapPatientDemographicsToEntity(PatientDemographicsModel patientDemographics, Patients patients, PHIEncryptedModel pHIEncryptedModel, TokenModel token, string action)
        {
            ...
            if (action == "add")
            {
                patients.CreatedBy = token.UserID;
                ...
                if (patientDemographics.PatientDiagnosis != null && patientDemographics.PatientDiagnosis.Count > 0)
                {
                    ...
                    patientDemographics.PatientDiagnosis.ForEach(a =>
                    {
                        ...
                        patientDiagnosis.CreatedBy = token.UserID;
                        patientDiagnosis.IsPrimary = true;
                        patientDiagnosis.IsActive = true;
                        patientDiagnosis.ICDID = a.ICDID;
        ...
        private Entity.User SaveUser(Patients entity, TokenModel token, string password)
        {
            ...
            requestUser.RoleID = _context.UserRoles.Where(m => m.RoleName.ToLower() == OrganizationRoles.Client.ToString().ToLower() && m.OrganizationID == token.OrganizationID && m.IsActive == true && m.IsDeleted == false).FirstOrDefault().Id; ;//4; 
                                                                                                                                                                                                                                                      //
